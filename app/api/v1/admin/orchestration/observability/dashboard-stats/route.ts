 ...
 * their own rows plus system-owned ones, the latter where the authorization
 * policy permits an unattributed read. Both models ask it now — executions
 * through `executionVisibilityWhere`, conversations through
 * `conversationVisibilityWhere` — so a fork narrowing `canRead` narrows this
 * whole page coherently rather than half of it.
 *
 * The conversation clause passes `excludeShared`: a thread merely shared with
 * this admin is still someone else's, and counting it among *their* active
 * conversations would overstate the number. That narrows, and is unrelated to
 * the policy question.
 *
...
export const GET = withAdminAuth(async (request, session) => {
  ...
  // owner and belongs to the deployment, so it counts — where the policy
  // permits it, which is now the same question the execution clause above asks.
  const conversationVisibility = conversationVisibilityWhere(session, {
    excludeShared: true,
  });
