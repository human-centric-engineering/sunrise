...
describe('POST /api/v1/admin/orchestration/agents/:id/versions/:versionId/restore', () => {
  ...
  it('restore applies enableImageInput and enableDocumentInput from the snapshot', async () => {
    ...
    // attachment toggles or a restore would silently revert the
    // wrong bit.
    vi.mocked(auth.api.getSession).mockResolvedValue(mockAdminUser());
    vi.mocked(prisma.aiAgent.findUnique).mockResolvedValue({ id: AGENT_ID } as never);
    vi.mocked(prisma.aiAgentVersion.findFirst)
      .mockResolvedValueOnce({
        ...
        agentId: AGENT_ID,
        version: 2,
        snapshot: { ...SNAPSHOT, enableImageInput: true, enableDocumentInput: false },
      } as never)
      .mockResolvedValueOnce({ version: 4 } as never);
    vi.mocked(prisma.aiAgent.update).mockResolvedValue({
      id: AGENT_ID,
    ...
    expect(prisma.aiAgent.update).toHaveBeenCalledWith({
      where: { id: AGENT_ID },
      data: expect.objectContaining({ enableImageInput: true, enableDocumentInput: false }),
    });
  });
