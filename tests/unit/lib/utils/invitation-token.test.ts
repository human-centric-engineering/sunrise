...
describe('validateInvitationToken()', () => {
  ...
  describe('valid token', () => {
    ...
    it('should query database with correct identifier', async () => {
      // Arrange
      const email = 'test@example.com';
      const token = randomBytes(32).toString('hex');
  ...
  describe('invalid token', () => {
    ...
    it('should log warning when token not found', async () => {
      ...
      // Assert
      expect(logger.warn).toHaveBeenCalledWith('Invitation token not found', { email });
    });
  ...
  describe('expired token', () => {
    it('should return false when token has expired', async () => {
      ...
      const email = 'user@example.com';
      const token = randomBytes(32).toString('hex');
      const verification = createMockVerification(email, token, -1); // Expired yesterday
    ...
    it('should log warning when token has expired', async () => {
      // Arrange
      const email = 'user@example.com';
      const token = randomBytes(32).toString('hex');
      const verification = createMockVerification(email, token, -1);
    ...
    it('should handle token expiring exactly now', async () => {
      ...
      const email = 'user@example.com';
      const token = randomBytes(32).toString('hex');
      const verification = createMockVerification(email, token, 0);
      verification.expiresAt = new Date(); // Expires right now
      ...
      // Assert: Should be invalid (expired tokens are not valid)
      expect(result).toBe(false);
    });
  });
