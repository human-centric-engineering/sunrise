...
// A genuinely cold dispatcher. The singleton is `globalThis`-backed and this
// worker may already have run a file that populated it, so the slot is cleared
// BEFORE the module is imported — otherwise the "empty map" this test depends
// on would be somebody else's full one, and it would pass without the fix.
...
const { __resetRegistrationForTests } = await import('@/lib/orchestration/capabilities/registry');
...
describe('executeToolCall on a cold process (#537)', () => {
  ...
  it('is a no-op on the second call — registration is idempotent', async () => {
    await expect(executeToolCall(step, ctx)).rejects.toThrow();
    const registeredCount = capabilityDispatcher.has(BUILT_IN_SLUG);

    await expect(executeToolCall(step, ctx)).rejects.toMatchObject({
      code: 'capability_inactive',
    });

    expect(registeredCount).toBe(true);
    expect(capabilityDispatcher.has(BUILT_IN_SLUG)).toBe(true);
  });
});
