...
| `epub-ingestion.ts` | Real `.epub` upload → `parseEpub` → chunk → store, via `POST /knowledge/documents` on a running server                                                                                             | Nothing — that is the point                                                                                  | **Point it at a production build** (`npm run build && PORT=3100 npm start`, then `SMOKE_BASE_URL=http://localhost:3100`). The EPUB path has broken in ways vitest could not see (#606), and the knowledge parsers have a history of working in dev and failing only once bundled — jsdom's ESM move did it once. Signs up a `smoke-test-epub` admin, verifies it in-DB (production requires email verification), and removes both rows. |
