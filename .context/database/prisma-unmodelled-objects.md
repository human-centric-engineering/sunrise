...
| A5    | `idx_knowledge_doc_file_hash_ready`         | partial unique index      | `ai_knowledge_document`  | `SELECT indexdef FROM pg_indexes WHERE indexname = 'idx_knowledge_doc_file_hash_ready'` — and the definition names `"orgId"`            |
...
| A7    | `idx_ai_knowledge_base_single_default`      | partial unique index      | `ai_knowledge_base`      | `SELECT indexdef FROM pg_indexes WHERE indexname = 'idx_ai_knowledge_base_single_default'` — and the definition names `"orgId"`         |
...
- **A5 partial unique** `(orgId, fileHash) WHERE status = 'ready'` (per org since `20260921120000_org_scoped_slugs`, §107 t-708; `(fileHash)` before). Allows retrying a failed upload of the same content without manual cleanup, but blocks two concurrent successful uploads of the same content in one org from creating duplicate ready rows. Two orgs may each hold the same file — a global index here failed the second org's upload at the end of ingestion with a violation from a table it cannot see into. Drop → duplicate ready rows accumulate; admin UI shows fake "two copies" of the same document. The probe also checks the definition names `orgId`, since the migration re-created the index under its old name.
...
- **A7 single-default partial unique** `(orgId) WHERE isDefault = true` (per org since `20260921120000_org_scoped_slugs`, §107 t-708; `(isDefault)` before). At most one row PER ORG may carry the default flag — the runtime invariant `getOrCreateDefaultKnowledgeBase()` and every upload path depend on; each org's first upload creates its own default. Drop → uploading code keeps going if a second `isDefault=true` row is created (e.g. by an admin SQL fix gone wrong), but new documents may route to whichever default the upsert happens to find by slug-natural-key.
