...
Scope enters a run only via `OrchestrationEngine.execute` (the fresh-run path), and the **rerun** endpoint inherits the original execution's scope. Runs whose rows are created directly — the scheduler, inbound/webhook triggers, and the `run_workflow` sub-workflow capability — do **not** set `scope` yet; propagating it to those paths is tracked in #384 (to land with scope population).
