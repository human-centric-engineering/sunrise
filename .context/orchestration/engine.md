...
The live API surfaces this as `currentRunningSteps: Array<{ stepId, label, stepType, startedAt, turnCount }>` (see `.context/api/orchestration-endpoints.md`). The detail view's `displayTrace` builder appends one synthesized "running" trace row per entry, so a `parallel` fan-out shows every branch simultaneously with its own ticking elapsed time. `turnCount` is the running-step row's `turns.length` — zero for single-shot step types, growing for `agent_call` / `orchestrator` / `reflect` as the model fires more iterations. The trace-row renders it as a "N turns" pill next to the duration so long agent calls show forward progress rather than looking frozen.
