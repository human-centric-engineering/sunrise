...
├── errors.ts                  # ExecutorError, PausedForApproval, BudgetExceeded
...
Each step carries `errorStrategy: 'retry' | 'fallback' | 'skip' | 'fail'` in its config (defaults to `fail`). The engine wraps each executor call in `runStepWithStrategy`:
