...
- **Versioning**: Every editable behavioural and operational field — General-tab metadata (`name`, `slug`, `description`, `isActive`, `visibility`, `retentionDays`), Model-tab config (provider, model, temperature, guard modes, attachment toggles, etc.), Knowledge-tab config (`knowledgeAccessMode`, tag grants, document grants), and Instructions-tab content (`systemInstructions`, topic boundaries, brand voice) — is captured in `VERSIONED_FIELDS` and creates an `AiAgentVersion` row on change. The model is **point-in-time**: each row stores the **post-save** state of every versioned field as a JSON snapshot (the config _as of_ that version) plus a tab-prefixed `changeSummary` (e.g. `"General: Description · Model: Temperature, Model"`). Every agent gets an explicit `v1` ("Initial configuration") at create/clone (and a seed backfills pre-existing agents), so the original is always a restorable entry. "Restore to version N" reproduces the agent exactly as it was at vN — labels match content and the newest row equals live; restore also reconnects the knowledge grants and `knowledgeAccessMode`. The version-history UI lazy-loads adjacent snapshots and renders a Before/After diff per save by diffing each row against its next-older neighbour (the oldest row shows the full initial config). System agents are restorable too, but `slug` / `systemInstructions` / `isActive` are skipped to preserve the read-only guards.
...
| `call_external_api`          | Outbound HTTP to allowlisted hosts; per-agent auth/idempotency/URL-prefix policy in `customConfig`; `${env:VAR}` template substitution for credential-bearing `forcedUrl`/`forcedHeaders`; multipart/form-data via `args.multipart` for endpoints requiring named file parts (e.g. Gotenberg) |
