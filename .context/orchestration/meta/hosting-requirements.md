...
| In-memory state        | Per-instance state includes: LRU rate-limit counters, scheduler claim windows, circuit breaker and budget mutex. The **MCP session map** used to be the worst of them and is gone (§39 t-718): MCP holds no session, so a multi-instance deploy no longer needs sticky routing for it | Single-instance: fine. Multi-instance: add Redis for shared rate limits + circuit breaker, and **enable sticky sessions for long-lived embed connections**                                                |
