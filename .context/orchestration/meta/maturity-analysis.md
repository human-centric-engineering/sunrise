...
**Sunrise advantages:** 15 step types (vs. LangGraph's node-based approach which is more flexible but less structured), per-step error strategies, dual cancellation paths, and step-output provenance — LLM/agent steps that emit `output.sources` get typed source attribution lifted onto the trace entry and rendered as colour-coded pills in the structured approval form and the trace viewer. The workflow-engine analogue of the chat-side citation envelope, with an opt-in guard rule that workflow authors paste into a `guard` step to enforce attribution. **Key gap:** Checkpoint recovery is limited to `human_approval` pauses — LangGraph's full state checkpoint on every super-step is the gold standard.
