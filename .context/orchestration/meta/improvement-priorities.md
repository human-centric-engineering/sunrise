...
**Critical files (for reference):** `lib/orchestration/knowledge/search.ts`, `prisma/migrations/00000000000000_baseline/migration.sql` (originally 2026-05-01 `add_knowledge_chunk_search_vector`, absorbed by the 2026-05-29 squash), `.context/orchestration/knowledge.md`.
...
- **Schema additions** on `AiWorkflowExecution`: `triggerSource VARCHAR(50)`, `triggerExternalId TEXT` (vendor attribution), `dedupKey TEXT` with `@@unique([dedupKey])`. New `AiWorkflowTrigger` model `(id, workflowId, channel, name, metadata, signingSecret, isEnabled, lastFiredAt, createdBy, …)` with `@@unique([channel, workflowId])`. Originally added 2026-05-07 by `add_inbound_triggers`, absorbed into `prisma/migrations/00000000000000_baseline/migration.sql` by the 2026-05-29 squash.
...
- **Critical files**: `lib/orchestration/inbound/{types,registry,bootstrap,index}.ts`, `lib/orchestration/inbound/adapters/{generic-hmac,postmark,slack}.ts`, `app/api/v1/inbound/[channel]/[slug]/route.ts`, `lib/orchestration/scheduling/scheduler.ts` (drainEngine export), `lib/security/rate-limit.ts` (inboundLimiter), `prisma/schema/` + `prisma/migrations/00000000000000_baseline/migration.sql` (originally 2026-05-07 `add_inbound_triggers`, absorbed by the 2026-05-29 squash), `.context/orchestration/inbound-triggers.md` (canonical doc).
...
**What we'd ship.** A new `lib/orchestration/mcp-client/` module mirroring the existing `lib/orchestration/mcp/` server shape. A new `call_mcp_tool` system capability with args `{ serverSlug, toolName, arguments }`. A new `AiMcpServer` table linking `(slug, serverUrl, authMode, authSecret, allowedAgentIds, isEnabled)` so admins register external servers once and bind them to specific agents via the existing capability-binding flow. Per-agent `customConfig.allowedMcpServers` allowlist (URL-slug matched, same posture as the SSRF protection on `external_call`) controls which servers each agent can hit. Connection pooling per `(serverSlug, authSecret)` with a 1-hour idle TTL — the figure the server's own session manager used before §39 t-718 deleted it, kept here because it was a reasonable number, not because anything still implements it. Auth modes mirror the HTTP fetcher: `none` / `bearer` / `api-key` / `query-param`. Tool-discovery is `list_tools` on first connection; results cached for the per-server-config TTL (default 1 hour) and refreshable by clicking "Refresh tools" on the server config page.
...
- **Schema.** Three additive columns on `AiCapability` — `quarantineState` (`active` / `quarantined-soft` / `quarantined-hard`, defaulting to `active`), `quarantineReason String?`, `quarantineUntil DateTime?`. Index on `quarantineState`. Migration (originally 2026-05-27 `add_capability_quarantine`, absorbed into the baseline by the 2026-05-29 squash) was hand-written to strip Prisma's auto-generated drift cleanup (the `searchVector` / `idx_message_embedding` drops and the `ai_conversation_inbound_key` rename) per the documented pattern in the schema comments — note the squash makes this concern historical, since the baseline now serializes the intended state directly.
