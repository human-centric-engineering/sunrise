...
| `paramProfile`         | openai-legacy, openai-reasoning, anthropic, gemini (nullable)                                                                                                      | Wire-level parameter convention. Drives whether the OpenAI-compatible provider sends `max_tokens` vs `max_completion_tokens` and whether `temperature` is honoured. When null, the runtime falls back to `deriveParamProfile()`.              |
