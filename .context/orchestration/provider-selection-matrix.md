...
| `capabilities`         | Subset of `MODEL_CAPABILITIES` (`chat`, `reasoning`, `embedding`, `audio`, `image`, `moderation`). `unknown` is catalogue-only — the matrix Zod schema rejects it. | Engine paths: `chat`/`reasoning` → `provider.chat()`; `embedding` → `provider.embed()`; `audio` → `provider.transcribe?()`. `image`/`moderation` are storage-only today (no engine path; UI surfaces a "Storage-only" indicator). |
