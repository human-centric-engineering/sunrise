...
`AiKnowledgeDocument.slug` is unique per org (`@@unique([orgId, slug])`, §107
t-708) — the stable cross-environment key that lets agent→document grants round-trip through agent export/import and
...
upload paths use `generateUniqueDocumentSlug`, which searches within the org the
caller runs in and appends `-2`, `-3`, … on the rare collision (a stale non-`ready`
row already holding the base slug); the seeded
...
**Every arm of that chain is filtered by the provider-eligibility seam** (`source: 'primary'`, `task: 'embeddings'`); the operator's pin at the top is not, on the same "we do not reroute a recorded decision" line the rest of the tree draws. A refused arm is skipped and the next is tried. Arm (4) has no `AiProviderConfig` row, so it answers to the reserved slug `env:openai` (`UNCONFIGURED_OPENAI_SLUG`, exported from `embedder.ts`) — the colon means it can never collide with a real slug, and without it that arm was the one destination no rule could name. If every arm is refused, `resolveProvider` throws `NoEligibleProviderError`, deliberately distinct from the `NoProviderConfiguredError` thrown when nothing is set up — one sends an operator to the rule, the other to the setup wizard, and `resolveEmbeddingAvailability()` carries that distinction out to the API as `providerState` so the admin banner prints the matching remedy. The arm that wins is logged at `debug` (per-call provenance); the pin drop-through that moved an install onto this chain is logged at `warn` by `resolveActiveEmbeddingConfig`, which is both where it happens and visible at production default level. See [llm-providers.md → Per-path coverage](./llm-providers.md#per-path-coverage). See "Active embedding model" at the top of this doc for the switching workflow.
