...
**Access-contributor seam (fork extension).** In the `restricted` branch only, the resolver also unions in documents/tags from any contributor a fork registers via `registerAgentAccessContributor(key, contributor)` — filled from the fork-owned `lib/app/knowledge-access-contributors.ts` scaffold (mirrors `registerContextContributor`). A contributor `(agentId) => Promise<{ documentIds?, tagIds? }>` lets a layer widen a restricted agent's set from a relationship it owns (module membership, team ACL, per-tenant grant), **composed live** rather than materialised onto the per-agent pivot (`AiAgentKnowledgeDocument` is `@@id([agentId, documentId])` with no provenance column, so a derived grant and an operator grant would be the same row — copy-down is clobber-or-leak). Guarantees: contributors run **after** the `full` short-circuit so they can only **widen**, never narrow, and never touch a `full` agent; a contributor that rejects _or_ throws synchronously is logged and ignored (the resolver stays always-safe-to-call); an **empty registry is byte-for-byte the stock behaviour**. Contributed `tagIds` join the existing tag→doc expansion. The composed set is cached like any other decision (60 s TTL), so a subsystem whose contributor data changes must call `invalidateAgentAccess(agentId)` for the affected agents — the same invalidation contract direct grants already follow. Reference: Daybreak composes a module's owned knowledge scope for every agent bound to the module.
