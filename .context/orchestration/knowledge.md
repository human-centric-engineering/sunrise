...
- `restricted` → (docs explicitly granted) ∪ (docs carrying any granted tag) ∪ (system-scoped seed docs) ∪ (docs/tags from any registered **access contributor** — see below).

**Access-contributor seam (fork extension).** In the `restricted` branch only, the resolver also unions in documents/tags from any contributor a fork registers via `registerAgentAccessContributor(key, contributor)` — filled from the fork-owned `lib/app/knowledge-access-contributors.ts` scaffold (mirrors `registerContextContributor`). A contributor `(agentId) => Promise<{ documentIds?, tagIds? }>` lets a layer widen a restricted agent's set from a relationship it owns (module membership, team ACL, per-tenant grant), **composed live** rather than materialised onto the per-agent pivot (`AiAgentKnowledgeDocument` is `@@id([agentId, documentId])` with no provenance column, so a derived grant and an operator grant would be the same row — copy-down is clobber-or-leak). Guarantees: contributors run **after** the `full` short-circuit so they can only **widen**, never narrow, and never touch a `full` agent; a contributor that rejects _or_ throws synchronously is logged and ignored (the resolver stays always-safe-to-call); a throwing **init** is logged and rolled back to the pre-init registry, since contributors only ever widen and a partial apply would hand an agent documents nobody granted it (see [Fork Init Seams](../architecture/fork-init-seams.md)); an **empty registry is byte-for-byte the stock behaviour**. Contributed `tagIds` join the existing tag→doc expansion. The composed set is cached like any other decision (60 s TTL), so a subsystem whose contributor data changes must call `invalidateAgentAccess(agentId)` for the affected agents — the same invalidation contract direct grants already follow. Reference: Daybreak composes a module's owned knowledge scope for every agent bound to the module.
...
**Implementation:** `embedChunks()` in `seeder.ts` is the shared primitive — the seed flow also calls it after inserting chunks. Both routes rate-limit via `adminLimiter`. The `hasActiveProvider` flag on the status endpoint is `true` when `resolveProvider()` actually resolves — `canResolveEmbeddingProvider()` in `embedder.ts` — so the admin UI can disable the "Generate Embeddings" button until embedding can genuinely run. It used to mean "an active `AiProviderConfig` row exists **or** `OPENAI_API_KEY` is set", which was the same thing until the chain started consulting the eligibility rule (see **Provider resolution** below); on a fork whose rule refuses every arm, the old check enabled the button for a run that could not succeed. A transient database error is not an answer of "no" — it propagates and the endpoint 500s.
...
**Every arm of that chain is filtered by the provider-eligibility seam** (`source: 'primary'`, `task: 'embeddings'`); the operator's pin at the top is not, on the same "we do not reroute a recorded decision" line the rest of the tree draws. A refused arm is skipped and the next is tried. Arm (4) has no `AiProviderConfig` row, so it answers to the reserved slug `env:openai` (`UNCONFIGURED_OPENAI_SLUG`, exported from `embedder.ts`) — the colon means it can never collide with a real slug, and without it that arm was the one destination no rule could name. If every arm is refused, `resolveProvider` throws "No permitted embedding provider", worded apart from the "no embedding provider configured" error so an operator is sent to the rule rather than to the setup wizard. The arm that wins is logged at `info`. See [llm-providers.md → Per-path coverage](./llm-providers.md#per-path-coverage). See "Active embedding model" at the top of this doc for the switching workflow.
