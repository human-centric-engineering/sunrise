...
> The **column** still carries `@default("anthropic")` (`prisma/schema/orchestration-agents.prisma`), so `.context/database/models.md` documenting that default is correct — it describes the database, not the API. Nothing relies on it: every `prisma.aiAgent.create()`/`upsert()` site passes `provider` explicitly, and the API layer now requires it. It is a latent instance of the same fail-open one layer further down, and removing it needs a migration. Optional fields include `rateLimitRpm` (Int, per-agent rate limit in requests/minute — null uses global default) and `runtimePromptManaged` (Boolean, default `false`) + `runtimePromptNote` (nullable String ≤ 2,000 chars) — an advisory, behaviour-neutral marker for agents whose system prompt is built in application code per call rather than read from the stored instruction fields; the runtime never reads it (see [`.context/admin/agent-form.md`](../admin/agent-form.md#runtime-built-prompt-honesty-flag)). New agents start with `systemInstructionsHistory: []` and `createdBy = session.user.id`. Slug collision → 409 `ConflictError`.
