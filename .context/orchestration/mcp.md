...
> **The advertised tool name can move when a capability is saved (#509).** `tools/list` advertises `customName ?? functionDefinition.name`, and `tools/call` resolves an incoming call by that advertised name. Since #509, every write to a capability forces `functionDefinition.name` to equal its `slug` — so for a tool exposed **without** a `customName`, an admin saving the capability renames it, and a client calling the old name gets `Unknown tool`. Capabilities created through the admin UI before that release diverged by default (`search-web` slug, `search_web` function name), so this is not a hypothetical for older installs. **Set `customName` on the `McpExposedTool` row to pin the external contract**; it takes precedence and is never touched by the invariant.
