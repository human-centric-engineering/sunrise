...
- **At `TENANCY_MODE=multi`, the session list and that terminate button are not scoped to an org, and neither is server-push** (declared defect, §108 t-716). `getActiveSessions()` filters on TTL alone, so the Sessions page shows an org admin every org's session ids, `apiKeyId`s and activity times; `DELETE …/sessions/:id` has no ownership check, so an id read off that page ends another org's session; and a `list_changed` notification reaches every connected org's SSE stream. `McpSession` carries no `orgId` yet. **Do not enable `stateful` on a multi-tenant install until t-716 lands** — on `single` it is not a concern, and `stateless` (the default) stores no session at all.
