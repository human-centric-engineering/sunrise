...
**Scoping.** The three execution-row cards (Running, Queued, Orphaned) are scoped to what the authenticated admin can see — their own runs plus system-owned ones (`userId IS NULL`: schedule- and inbound-triggered, see [#502](https://github.com/human-centric-engineering/sunrise/issues/502)), the latter where the authorization policy permits an unattributed read — so the numbers match what the executions list, force-fail, and lease inspector routes will let them act on. All four share the clause from `executionVisibilityWhere` (`lib/orchestration/access/execution-access.ts`); scheduled runs are exactly the rows this dashboard exists to catch stuck, so dropping the system arm would gut it.

Without that match an admin would see "5 running" on the dashboard but find only 2 on their own list, with no way to drill into the other 3.

**A narrowing fork does that to itself, and it is worth knowing before you register such a policy.** A `canRead` that refuses `'unattributed'` reads leaves Running / Queued / Orphaned counting only the caller's own runs: a scheduled run wedged with an expired lease shows as zero on those three cards, and `force-fail` — the operator's escape hatch — returns 404, because it is gated solely on `adminCanViewExecution` with no second grant of any kind. So the run cannot be seen, drilled into, or **killed by hand** by anybody.

**It is still recovered automatically, and that is the difference between an annoyance and an outage.** `reapZombieExecutions` (`lib/orchestration/engine/execution-reaper.ts`, registered as the `zombieReaper` platform job on a 5-minute interval) filters on `status` and a time cutoff only — no `userId`, no policy — so it force-fails a stuck `running` row after 30 minutes, a `pending` one after an hour, and an abandoned approval after 7 days, on a narrowing fork exactly as on a default install. What a narrowing fork loses is **operator-initiated** recovery inside that window, not recovery — for the callers its policy refuses. The 7-day case is the approvals gate, which nothing sweeps sooner, and it was settled with t-690 the same way as this one: the platform adds no second grant, and the fork keeps a principal its own policy admits to ownerless executions.

So a fork narrowing this arm keeps a vendor-level operator role its own policy admits, and an engineer in that role sees the cards and can `force-fail` inside the window rather than waiting out the reaper. `checkOwnerlessReachability` (`lib/auth/orphan-reads.ts`), run over the fork's policy and principals, reports the `execution` kind when no such principal exists. Under a customer tier that role is the identity work's (§106) to define — and once `canRead` sees `scope.org`, an org's own scheduled runs stop being ownerless for that org's admins.

The Provider in-flight card is process-wide on purpose: the in-flight counter has no user attribution at the proxy boundary, and operators read it as "the worker my admin tab hit is currently handling N calls." For a genuinely global "is the engine healthy?" lens across users, see priorities-doc #41 (Workflow-execution health dashboard) — explicitly a separate concern.
