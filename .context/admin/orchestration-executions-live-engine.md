...
**A narrowing fork does that to itself, and it is the sharpest consequence of putting this behind the policy.** A `canRead` that refuses `'unattributed'` reads leaves Running / Queued / Orphaned counting only the caller's own runs: a scheduled run wedged with an expired lease shows as zero on every admin's dashboard, and `force-fail` — the operator's escape hatch, gated solely on `adminCanViewExecution` with no second grant of any kind — returns 404 for all of them. The run cannot be seen, drilled into, or killed by anybody. This is worse than the approvals gap tracked in t-690, which at least leaves a named approver able to act if they learn the id; here there is no fallback path at all. A fork that narrows this arm needs a vendor-level operator role that the policy still admits, or it has no recovery story for stuck system-owned runs.
