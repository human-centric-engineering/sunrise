...
**It is still recovered automatically, and that is the difference between an annoyance and an outage.** `reapZombieExecutions` (`lib/orchestration/engine/execution-reaper.ts`, registered as the `zombieReaper` platform job on a 5-minute interval) filters on `status` and a time cutoff only — no `userId`, no policy — so it force-fails a stuck `running` row after 30 minutes, a `pending` one after an hour, and an abandoned approval after 7 days, on a narrowing fork exactly as on a default install. What a narrowing fork loses is **operator-initiated** recovery inside that window, not recovery. Worth fixing, not worth panicking about; and for the 7-day case it is the approvals gap in t-690 that bites first, since nothing sweeps a paused run sooner.

A fork narrowing this arm probably still wants a vendor-level operator role its own policy admits, so an engineer can act inside the window rather than waiting out the reaper.
