...
- **In use** — toggle chip that hides rows with no bound agents (filters by `agents?.length > 0`).
...
| Capabilities                                 | `model.capabilities[]`                   | One pill per stored capability (`Chat` / `Reasoning` / `Embedding` / `Audio` / `Image` / `Moderation`). Rows whose only capabilities are image/moderation render a muted `Storage-only` indicator — the orchestration engine has no runtime path for those yet. |
