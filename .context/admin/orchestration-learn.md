...
- **Suggested-prompts disclosure**: Once the conversation has at least one message, the starter grid is hidden but a collapsible "Suggested prompts" disclosure appears beneath the top action cluster. Default closed; opening it reveals the same five prompts and the shuffle icon (when present). Quiz tab gets the disclosure too with its four static prompts, but no shuffle since `QUIZ_PROMPTS` isn't a pool
