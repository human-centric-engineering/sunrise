...
**Scoping.** Every query except the cost-log count is scoped to what the caller can see: their own rows **plus system-owned ones** (`userId = null` — schedule- and inbound-triggered, [#502](https://github.com/human-centric-engineering/sunrise/issues/502)). **Both models take that second arm from the authorization policy** — open on a default install, narrowable by a fork, and narrowed coherently across the whole page rather than half of it. Executions share the `executionVisibilityWhere` clause with the executions list and the [live-engine dashboard](./orchestration-executions-live-engine.md); conversations use `conversationVisibilityWhere(session, { excludeShared: true })` — the owner-or-system arms only, since a conversation merely _shared_ with this admin is still someone else's and would overstate their active count. That exclusion narrows and is unrelated to the policy question. Dropping the system arm makes this page report a healthy deployment while the live-engine dashboard shows the same scheduled runs failing — two admin surfaces disagreeing about the same rows.
...
- **Timeline strip**: Gantt-style horizontal bar per step on a shared wall-clock axis, so concurrent steps overlap visibly. Slow outliers (≥ p90 in traces with ≥ 5 entries) and failed bars colour-coded; awaiting-approval bars amber-striped. **Parallel-branch wait segment** — a branch that finishes before the slowest sibling renders as a coloured processing portion followed by a hashed grey wait segment that extends to the fork's join time (the latest sibling's end). While any sibling is still running the wait grows live, then freezes once the whole fork settles. The duration column shows the wait as a small `+Xs wait` line under the processing time, and the matching `ExecutionTraceEntryRow` adds an inline `+N ms waited for siblings` meta line. Click → ring-highlights and scrolls to the matching trace row below.
...
- **Step timeline**: per-trace-entry `ExecutionTraceEntryRow` with header (status pill, label, type, **provider · model chip**, duration, **latency breakdown** "LLM xxx ms · other yyy ms", token split, cost). Expanded body shows **input + output side-by-side** and a **per-call cost sub-table** populated from `costEntries[]` grouped by `stepId`. **Each Input/Output pane** has a **Wrap** toggle (swaps `JsonPretty`'s `whitespace-pre` + `overflow-x-auto` default for `whitespace-pre-wrap` + `break-words` so long string values wrap at the right margin while JSON indentation survives — hidden for markdown content, which wraps naturally) and an **Expand** button that opens a `max-w-5xl` Dialog with the same data, its own wrap toggle defaulted ON, Copy, and the parent's toolbar slot so the input pane's "Show resolved" toggle still works inside the dialog. Inline and dialog wrap states are independent.
