...
interface WorkflowDefinition {
  ...
  errorStrategy: 'retry' | 'fallback' | 'fail'; // Workflow-level default
...
Prompts in `llm_call`, `route`, `reflect`, and `plan` steps support template variables resolved by `llm-runner.ts`:
...
4. **Per-type config** — `human_approval` needs `prompt`, `tool_call` needs `capabilitySlug`
...
1. **`types/orchestration.ts`** — add the literal to `KNOWN_STEP_TYPES` / `WorkflowStepType`
...
6. **Unit test** — write tests under `tests/unit/lib/orchestration/engine/executors/`
