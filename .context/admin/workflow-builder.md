...
- **`retry`** (`edge-types/retry-edge.tsx`) — dashed amber line (`#f59e0b`, `strokeDasharray: 6 3`) with a label badge at the midpoint showing the condition and retry count (e.g. "fail (retry ×2)"). Used for bounded retry back-edges where `ConditionalEdge.maxRetries > 0`. All other edges use the React Flow `default` type.

**Guard retry config.** When a guard step's `failAction` is `block`, the guard editor shows a "Retry on failure" number input (0–10). Setting this to > 0 creates a bounded retry back-edge: the guard's Fail output, when wired to an upstream step, will retry that step up to `maxRetries` times. The failure context (`__retryContext.failureReason`) is forwarded so the LLM can learn from its mistake. See [engine.md → Bounded retry loops](../orchestration/engine.md#bounded-retry-loops).
