...
| Auto-fallback (Q15)                               | **Fixed via an eligibility seam**: fallback only within `resolveEligibleProviders(ctx)`; default = today's behaviour at `single`, deny-by-default at `multi`. **Shipped 2026-09-03** as `registerProviderEligibility` (`lib/app/providers.ts`). Scoped to fallbacks; whether a per-org rule should also constrain the auto-picked PRIMARY provider is open and belongs to the per-org work — see below. |
