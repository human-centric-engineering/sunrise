...
| Auto-fallback (Q15)                               | **Fixed via an eligibility seam**: fallback only within `resolveEligibleProviders(ctx)`; default = today's behaviour at `single`, deny-by-default at `multi`. **Shipped 2026-09-03** as `registerProviderEligibility` (`lib/app/providers.ts`), covering the auto-picked primary AND both fallback lists — every provider choice made on the caller's behalf. An EXPLICIT `agent.provider` is deliberately not overridden at runtime; enforcing that belongs at write time (do not offer a provider the org has not approved), which is per-org work. Both layers are required: write-time validation cannot reach agents stranded by a policy that changed after they were configured, nor writes that bypass the form. |
