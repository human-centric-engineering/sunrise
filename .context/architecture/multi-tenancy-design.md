...
| Auto-fallback (Q15)                               | **Fixed via an eligibility seam**: fallback only within `resolveEligibleProviders(ctx)`; default = today's behaviour at `single`, deny-by-default at `multi`. **Shipped 2026-09-03** as `registerProviderEligibility` (`lib/app/llm-providers.ts`), covering the auto-picked primary AND both fallback lists at the agent-binding resolver, plus the second chokepoint — a workflow step with no `modelOverride`, and knowledge keyword enrichment, which resolve a provider from the model registry without reaching the resolver and so consult the seam directly via `isProviderEligible`. Every path is listed as covered or not in the coverage table in `.context/orchestration/llm-providers.md`; still uncovered by design are an explicit step `modelOverride` and the `EVALUATION_DEFAULT_*` env vars. An EXPLICIT `agent.provider` is deliberately not overridden at runtime; enforcing that belongs at write time (do not offer a provider the org has not approved), which is per-org work. Both layers are required: write-time validation cannot reach agents stranded by a policy that changed after they were configured, nor writes that bypass the form. |
