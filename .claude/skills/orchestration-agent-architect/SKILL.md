...
| Reusable persona / voice / guardrails | `AiAgentProfile` — shared rows that multiple agents inherit from; each agent has an optional `profileId` and may override or append per-field via `personaMode` / `voiceMode` / `guardrailsMode`. The runtime `resolveEffectivePrompt(agent, profile)` composes the final system prompt from profile + agent fragments before either chat or workflow `agent_call` sends. Treat profiles as the way to enforce a single brand voice across a fleet of differently-bound agents without duplicating systemInstructions. |
| Reasoning depth (17)                  | Per-agent `AiAgent.reasoningEffort` + per-step `reasoningEffort` on every LLM-bearing workflow step (`llm_call` / `plan` / `route` / `evaluate` / `reflect` / `guard.llm` / `agent_call` / `orchestrator` / `supervisor`). Honoured by reasoning models (o-series / gpt-5 / Claude 4 thinking), silently dropped elsewhere. Default null = no `reasoning_effort` parameter sent.                                                                                                                                       |
...
**Conversation provenance is built-in.** Every chat message persists the resolved `agentVersionId`, `workflowVersionId` (when workflow-fired), `modelId`, `providerSlug`, citation chunk hashes, and a redacted summary of every capability call made on that turn. The full bundle is downloadable via `GET /admin/orchestration/conversations/:id/provenance` in JSON or deterministic Markdown. When designing for regulated-domain partners (legal, financial, health, planning), the audit-trail story is already there — design choices should preserve it, not work around it. Capabilities that handle PII must set `processesPii = true` and override `redactProvenance` (the registry enforces the pairing at registration time).
